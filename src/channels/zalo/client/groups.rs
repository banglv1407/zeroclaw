...
impl ZaloGroups {
    ...
    pub async fn get_groups(&self, cookie: &str) -> Result<Vec<ZaloGroup>> {
        if self.group_poll_url.is_empty() {
            return Err(anyhow::anyhow!(
                "Group poll service URL not set. Login first to get zpw_service_map_v3.",
            ));
        }
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("Get groups failed: {e}"))?;

        let body: serde_json::Value = response
            .json()
            .await
            .map_err(|e| anyhow::anyhow!("Invalid groups response: {e}"))?;
    ...
    pub async fn get_group_info(&self, group_id: &str, cookie: &str) -> Result<ZaloGroup> {
        if self.group_url.is_empty() {
            return Err(anyhow::anyhow!(
                "Group service URL not set. Login first to get zpw_service_map_v3.",
            ));
        }
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("Get group info failed: {e}"))?;

        let body: serde_json::Value = response
            .json()
            .await
            .map_err(|e| anyhow::anyhow!("Invalid group response: {e}"))?;
