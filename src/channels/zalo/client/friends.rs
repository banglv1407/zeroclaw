...
impl ZaloFriends {
    ...
    pub async fn get_friends(&self, cookie: &str) -> Result<Vec<ZaloUser>> {
        if self.profile_url.is_empty() {
            return Err(anyhow::anyhow!(
                "Profile service URL not set. Login first to get zpw_service_map_v3.",
            ));
        }
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("Get friends failed: {e}"))?;

        let body: serde_json::Value = response
            .json()
            .await
            .map_err(|e| anyhow::anyhow!("Invalid friends response: {e}"))?;
    ...
    pub async fn get_user_info(&self, user_id: &str, cookie: &str) -> Result<ZaloUser> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("Get user info failed: {e}"))?;

        let body: serde_json::Value = response
            .json()
            .await
            .map_err(|e| anyhow::anyhow!("Invalid user response: {e}"))?;
    ...
    pub async fn send_friend_request(
        ...
        if self.friend_url.is_empty() {
            return Err(anyhow::anyhow!("Friend service URL not set. Login first.",));
        }
        ...
        self.client
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("Send friend request failed: {e}"))?;
