...
impl ZaloBusiness {
    ...
    pub async fn get_catalog(&self, access_token: &str) -> Result<Vec<ZaloCatalog>> {
        let response = self
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("Get catalog failed: {e}"))?;

        let body: serde_json::Value = response
            .json()
            .await
            .map_err(|e| anyhow::anyhow!("Invalid catalog response: {e}"))?;
    ...
    pub async fn send_oa_message(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("OA send failed: {e}"))?;

        let result: serde_json::Value = response
            .json()
            .await
            .map_err(|e| anyhow::anyhow!("OA send response error: {e}"))?;
